 ...
 *   <tools>{content}</tools>
 */
export const inlineTag = (tag, content) => {
  if (!content) return '';
  return [`<${tag}>`, content, `</${tag}>`].join('');
};
// Sections split by double newlines
export const sections = (sections) => sections.filter(Boolean).join('\n\n');
 ...
 */
export function formatTextFromMessages(messages, options) {
...
const removeEscapeSelection = (editor, text) => {
  let newText = text
